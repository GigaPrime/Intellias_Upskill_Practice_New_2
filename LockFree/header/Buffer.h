...
private:
	...
	alignas(64) std::atomic<std::size_t> tail_{ 0 };        // since modern CPUs cache lines are typically 64 bytes, aligning these atomic variables to 64 bytes 
                                                            // helps to ensure that they do not share a cache line, which can lead to performance degradation due to false sharing.
